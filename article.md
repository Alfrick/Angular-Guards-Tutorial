...
* **CanActivate**— adds a check for route navigation before a component is deployed. With this guard, you can permit or deny access to a feature based on the credentials of the user.
* **CanActivateChild** –adds a check for the route children before a component is deployed. It works just like the **CanActivate** guard.
* **CanDeactivate**—adds a check for navigation away from the current route, such as leaving a page without filling the form.
* **Resolve**—adds a check for retrieving or loading route data before the route is activated.
* **CanLoad**—adds a check for navigating to a feature module that loaded asynchronously.
...
As you can see from the code above, the `CanActivate` and `CanActivateChild` interfaces are implemented by declaring the `canActivate` and `canActivateChild` methods respectively (notice the difference in capitalization in “c”). And, as earlier explained, the method returns `true` or `false`—depending on the desired navigation process.
